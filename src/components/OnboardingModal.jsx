...
import { X, Globe, User, Shield, Check } from 'lucide-react';
          ...
          <div className="relative z-0">
            {step === 1 && (
              <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
                ...
                <h2 className="text-3xl font-extrabold text-white mb-3 font-display">Welcome to ElectIQ</h2>
                <p className="text-text-muted mb-8 text-sm leading-relaxed">
                  Your personalized AI guide to elections worldwide. Before we start, let's tailor your experience.
                </p>
            ...
            {step === 2 && (
              <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
                <div className="flex items-center gap-3 mb-8">
                  ...
                  <div>
                    <h2 className="text-xl font-extrabold text-white font-display">Select Region</h2>
                    <p className="text-xs text-text-muted">Choose the election you're tracking.</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3 mb-8">
                  {['india', 'usa', 'uk', 'australia'].map(c => (
                    ...
                    >
                      <span className="font-bold capitalize">{c === 'usa' ? 'USA' : c === 'uk' ? 'UK' : c}</span>
                      {country === c && <Check className="w-4 h-4 text-accent-purple" />}
                    </button>
                  ))}
                </div>

                <div className="flex justify-between gap-3">
                  <button onClick={() => setStep(1)} className="px-6 py-3 bg-dark-card border border-dark-border rounded-xl text-sm font-bold text-white hover:bg-dark-border transition-colors">Back</button>
                  <button onClick={() => setStep(3)} className="flex-1 py-3 bg-accent-purple text-white rounded-xl text-sm font-bold shadow-premium hover:bg-accent-purple/80 transition-all">Continue</button>
                </div>
              </motion.div>
            )}
            ...
            {step === 3 && (
              <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
                <div className="flex items-center gap-3 mb-8">
                  ...
                  <div>
                    <h2 className="text-xl font-extrabold text-white font-display">Select Role</h2>
                    <p className="text-xs text-text-muted">Customize the guidance you receive.</p>
                  </div>
                </div>

                <div className="space-y-3 mb-8">
                  {[
                    { id: 'voter', label: 'Voter', desc: 'Schedules, polling locations, ID rules.' },
                    { id: 'candidate', label: 'Candidate', desc: 'Filing rules, campaigning laws, forms.' }
                  ].map(r => (
                    <button
                      key={r.id}
                    ...
                    >
                      <div className={`w-5 h-5 mt-0.5 rounded-full border-2 flex items-center justify-center ${
                        role === r.id ? 'border-accent-purple' : 'border-dark-border'
                      }`}>
                        {role === r.id && <div className="w-2.5 h-2.5 bg-accent-purple rounded-full" />}
                      </div>
                      <div>
                        <span className={`block font-bold mb-1 ${role === r.id ? 'text-white' : 'text-text-primary'}`}>{r.label}</span>
                        <span className="text-xs text-text-muted leading-tight block">{r.desc}</span>
                      </div>
                    </button>
                  ))}
                </div>
                ...
                <div className="flex justify-between gap-3">
                  <button onClick={() => setStep(2)} className="px-6 py-3 bg-dark-card border border-dark-border rounded-xl text-sm font-bold text-white hover:bg-dark-border transition-colors">Back</button>
                  <button onClick={handleComplete} className="flex-1 py-3 bg-accent-purple text-white rounded-xl text-sm font-bold shadow-premium hover:bg-accent-purple/80 transition-all">Finish Setup</button>
                </div>
              </motion.div>
            )}
          </div>
