...
const CountdownTimer = () => {
  ...
  useEffect(() => {
    ...
    const calculate = () => {
      ...
      if (diff <= 0) return { days: 0, hours: 0, minutes: 0, seconds: 0, isActive: false };
      return {
      ...
      <div className="flex gap-2">
        {units.map((unit) => (
          <div key={unit.label} className="text-center">
            ...
            <span className="text-[8px] text-text-muted mt-1 block font-bold uppercase tracking-widest">{unit.label}</span>
          </div>
        ))}
      </div>
    </div>
