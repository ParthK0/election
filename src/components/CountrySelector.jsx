            ...
            {countries.map((c) => (
              <button
                ...
                className={`w-full flex items-center justify-between px-4 py-3 text-left text-sm transition-colors ${
                  country === c.id
                    ? "bg-accent-purple/10 text-accent-purple font-bold border-l-2 border-accent-purple"
                    : c.disabled
                      ? "text-text-muted/40 cursor-not-allowed"
                      : "text-text-primary hover:bg-dark-card-2 border-l-2 border-transparent"
                }`}
              >
                <div className="flex items-center gap-3">
                  <span className={c.disabled ? "opacity-40 grayscale" : ""}>
                    {c.flag}
                  </span>
                  <span>{c.name}</span>
                </div>
                {c.disabled && (
                  <span className="text-[8px] font-bold uppercase tracking-widest text-text-muted/50 border border-dark-border px-1.5 py-0.5 rounded">
                    Soon
                  </span>
                )}
              </button>
