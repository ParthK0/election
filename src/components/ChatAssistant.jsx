  ...
  const getSourceLinks = (content) => {
    ...
    if (country.toLowerCase() === 'india') {
      if (lowerContent.includes('register') || lowerContent.includes('voter')) {
        sources.push({ label: 'Voter Service Portal', url: 'https://voters.eci.gov.in' });
      }
      if (lowerContent.includes('candidate') || lowerContent.includes('affidavit')) {
        sources.push({ label: 'ECI Affidavits', url: 'https://affidavit.eci.gov.in' });
      }
    } else {
      ...
      if (lowerContent.includes('electoral')) {
        sources.push({ label: 'National Archives', url: 'https://www.archives.gov/electoral-college' });
      }
    }
    return sources;
  };
    ...
    <div className="flex flex-col h-[650px] bg-dark-surface border border-dark-border rounded-2xl overflow-hidden shadow-premium">
      ...
      <div className="px-6 py-4 flex items-center justify-between border-b border-dark-border bg-dark-card/50 backdrop-blur-md">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-bold text-white tracking-tight">ElectIQ AI</h3>
            <div className="flex items-center gap-1.5">
              <span className="w-1.5 h-1.5 rounded-full bg-accent-green animate-pulse"></span>
              <p className="text-[10px] font-bold text-text-muted uppercase tracking-widest">{country} Intelligence Active</p>
            </div>
          </div>
        </div>
            ...
            >
              <div className={`flex gap-3 max-w-[85%] ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                <div className={`w-8 h-8 rounded-full shrink-0 flex items-center justify-center ${
                  msg.role === 'user' ? 'bg-accent-purple' : 'bg-dark-card border border-dark-border'
                }`}>
                  {msg.role === 'user' ? <User className="w-4 h-4 text-white" /> : <Sparkles className="w-4 h-4 text-accent-purple" />}
                </div>
                <div className="space-y-2">
                  <div className={`p-4 rounded-2xl text-sm leading-relaxed shadow-premium ${
                    msg.role === 'user' 
                      ...
                      {getSourceLinks(msg.content).length > 0 && (
                        <div className="flex items-center gap-2">
                          ...
                          {getSourceLinks(msg.content).map((s, idx) => (
                            <a key={idx} href={s.url} target="_blank" rel="noreferrer" className="text-[10px] font-bold text-accent-purple hover:underline">
                              {s.label}
                            </a>
                          ))}
                        </div>
